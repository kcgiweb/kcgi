          ...
          {navConfig.map((nav, idx) => (
            ...
            >
              <div className="px-2 xl:px-3">
                {nav.mega ? (
                  <button className={`py-2 font-bold text-[11px] xl:text-xs uppercase tracking-wider transition-all flex items-center gap-1 whitespace-nowrap border-b-2 ${activeMenu === nav.title || location.pathname.startsWith(nav.path) ? 'border-secondary text-secondary' : 'border-transparent hover:text-secondary'}`}>
                    {nav.title} <ChevronDown className={`w-3 h-3 transition-transform ${activeMenu === nav.title ? 'rotate-180' : ''}`} />
                  </button>
                ) : (
                  <Link to={nav.path} className={`py-2 font-bold text-[11px] xl:text-xs uppercase tracking-wider transition-all whitespace-nowrap border-b-2 ${location.pathname === nav.path ? 'border-secondary text-secondary' : 'border-transparent hover:text-secondary'}`}>
                    {nav.title}
                  </Link>
                )}
              </div>
